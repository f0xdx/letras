 ...
 * 
 */
public class RdpConsole implements Runnable {
	...
	// defaults
	private static final String RDP_NAME = "raw data processor";
	...
	public RdpConsole() {
		...
		// report if it is not
		if (this.isAvailable()) logger.logp(Level.FINE, "RdpConsole", "RdpConsole", "console available");
		else logger.logp(Level.FINE, "RdpConsole", "RdpConsole", "console not available");
	}
	...
	private char readCommand() {
		String input = this.commandLine.readLine(CONSOLE_PROMPT, RDP_NAME);
	...
	private void helpMessage() {
		if (!this.isAvailable()) {
			logger.logp(Level.FINE, "RdpConsole", "printHelpMessage", "console unavailable");
			return;
		}
	 ...
	 */
	private void quit() {
		// stop the raw data processing stage
		RawDataProcessor.getInstance().stop();
	}
