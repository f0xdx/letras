...
class ChatServiceSR extends Service implements IReceiver
{
  ChatServiceSR()
  {
  }
  ...
  public void received(Message msg, MessageContext ctx)
  {
    ...
    TypedArray trace = msg.getMap("rs", "passive").getArray("trace", null);
    if (trace!=null)
    {
      System.out.println("(trace: "+trace+")");

      TypedMap map = new TypedMap();
      map.putString("ln", "reply from "+Mundo.getNodeId());
      RSSubscription.sendTo(publisher, trace, new Message(map));
    }
  }
...
//    Mundo.setConfig(NodeConf.getMap());
    ...
    ChatServiceSR cs = new ChatServiceSR();
