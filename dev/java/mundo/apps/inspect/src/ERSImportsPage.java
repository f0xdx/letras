...
class ERSImportsPage extends Page implements AsyncCall.IResultListener
{
  ERSImportsPage(DoDebugService ds)
  {
    doDebugService=ds;
  }
  @Override
  String getURI()
  {
    return "ers-imports:"+doDebugService._getPublisher().getChannel().getName();
  }
  @Override
  void load()
  {
    AsyncCall ac=doDebugService.getERSImports(DoObject.CREATEONLY);
  ...
  public void resultReceived(AsyncCall c)
  {
    ...
    try
    {
      imports = (List<DebugService.ERSImportEntry>)c.getObj();
      if (imports==null)
        throw new NullPointerException();
      TableModel tableModel=new ERSImportsTableModel(imports);
  ...
  {
    window.open(new ChannelPage(imports.get(table.getSelectedRow()).channelName), mod);
  }

  private DoDebugService doDebugService;
  private List<DebugService.ERSImportEntry> imports;
